...
const createExecutionStep = async (req, res) => {
  try {
    ...
    if (!execution_id || step_index === undefined || !step_name || !step_type || !input_payload || !status) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    const newExecutionStep = await executionStepRepository.createExecutionStep(req.body);
...
const getAllExecutionSteps = async (req, res) => {
  try {
    const executionSteps = await executionStepRepository.getAllExecutionSteps();
...
const getExecutionStepById = async (req, res) => {
  try {
    const { id } = req.params;
    const executionStep = await executionStepRepository.getExecutionStepById(id);
...
const deleteExecutionStepById = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedExecutionStep = await executionStepRepository.deleteExecutionStepById(id);

    if (!deletedExecutionStep) {
      return res.status(404).json({ error: 'Execution step not found' });
    }

    res.status(200).json({ message: 'Execution step deleted successfully', executionStep: deletedExecutionStep });
  } catch (error) {
